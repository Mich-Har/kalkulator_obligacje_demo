...
const [state, setState] = React.useState({
  ...
  // Tryby: "simple" (jedna wartość) lub "advanced" (rok po roku)
  inflationMode: 'simple',
  inflationByYear: [4.5,4.0,3.8,3.5,3.3,3.1,3.0,2.9,2.8,2.8,2.7,2.7],
  inflationAfterYear12: 2.5,

  nbpRateMode: 'simple',
  nbpRateByYear: [5.75,5.25,4.75,4.25,4.0,3.75,3.75,3.75,3.75,3.75,3.75,3.75],

  wibor6mMode: 'simple',
  wibor6mByYear: [5.40,4.90,4.40,4.00,3.75,3.60,3.60,3.60,3.60,3.60,3.60,3.60],

  depositRateMode: 'simple',
  depositRateByYear: [5.00,4.80,4.50,4.20,4.00,3.80,3.80,3.80,3.80,3.80,3.80,3.80],

  // Produkty — pełna lista z DEFAULT_PRODUCTS (patrz PRD sekcja Model danych)
...
**Logika oprocentowania rok po roku (dla każdego produktu):**
```
rok 1: initialRate
rok N (N>1):
  inflacja    → inflationByYear[N-1] + margin
  nbp_ref     → nbpRateByYear[N-1] + margin
  wibor6m     → wibor6mByYear[N-1] + margin
  fixed       → fixedRate (stałe, nie zmienia się)
```

**Kapitalizacja roczna (dla produktów z `capitalizationEveryMonths: 12`):**
```
value[y] = value[y-1] * (1 + rate[y] / 100)
```

**Koszt wcześniejszego wykupu** — odejmij `earlyRedemptionFee * bondCount` jeśli horyzont < maturityMonths.

**Dla LOK i KOS** — stałe oprocentowanie `fixedRate` przez cały horyzont, bez indeksacji.

**Dla INF** — wartość = `initialCapital * cumulativeInflation[horizon]` (czyste śledzenie inflacji).
